...
Update the consolidated migration report at `migration-reports/migration-<filename>.md`. Find the `<!-- PHASE:PERFORMANCE -->` marker and replace everything from that marker through to the next `---` separator with **Part 3: Performance Analysis**.

This makes the operation **idempotent** — re-running performance analysis replaces the previous section without duplicating content.
...
### 3.5 Configuration Recommendations
...
1. Analyze each file, updating its consolidated report
