...
Update the consolidated migration report at `migration-reports/migration-<filename>.md` (see [Report Output](#report-output) below).
...
Update the consolidated migration report at `migration-reports/migration-<filename>.md`. Find the `<!-- PHASE:M-LANGUAGE -->` marker and replace everything from that marker through to the next `---` separator with **Part 4: Power BI M-Language**.

This makes the operation **idempotent** — re-running M-language conversion replaces the previous section without duplicating content.

Replace the placeholder with this structure:

```markdown
<!-- PHASE:M-LANGUAGE -->
## Part 4: Power BI M-Language
...
2. Update each file's consolidated migration report
